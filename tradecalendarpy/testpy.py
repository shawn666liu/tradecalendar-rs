...
def test_calendar():
    ...
    sql = "SELECT date,morning,trading,night,next FROM futuredb.calendar WHERE date>='2024-01-01' ORDER BY date"
    cal = calendar.get_calendar(
        dburl, sql, csv_file=None, start_date=dt.date(2024, 1, 1)
    )
